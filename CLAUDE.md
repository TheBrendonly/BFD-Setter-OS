...
| `npx tsc --noEmit` inside `frontend/` | **True no-op, exit 0 always.** `frontend/tsconfig.json` has `"files": []` and only project references, which `tsc` does not follow without `-b`. A green run here proves nothing. |
| `npx tsc --noEmit -p tsconfig.app.json` inside `frontend/` | The ONLY typecheck that covers anything. Covers `frontend/src` only. **Currently reports 21 pre-existing errors in 10 files** (types.ts drift vs the live schema, `Json` mismatches, TS2589 depth blowups). Compare error counts before/after; do not expect zero. Needs ~60s and enough free RAM: it has been SIGTERM'd under memory pressure on this box. |
...
**For FRONTEND changes, run the headless render smoke** (memory `feedback_frontend_verify_render_smoke`), not
tsc/build: a GATE-A change once white-screened production while build and tsc were both green.
