-- F17 phase 1 — per-client call-recording disclosure toggle.
--
-- Engine-side boolean. make-retell-outbound-call + retell-inbound-webhook inject
-- a {{recording_disclosure}} dynamic variable ("required"/"not_required") on each
-- call; the spoken LINE itself is PU-6 (Brendan authors it in the prompt to
-- reference the variable). Non-secret, so surfaced through clients_public for the
-- agency toggle to read. Applied to LIVE prod via the Management API (this repo
-- has no migration runner; file is for record).

ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS recording_disclosure_enabled boolean NOT NULL DEFAULT false;
