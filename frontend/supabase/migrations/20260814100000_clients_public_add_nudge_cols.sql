-- 1B fix — expose the per-client nudge columns via clients_public.
--
-- The agency settings card (ClientNudgeSettingsCard) reads its config from
-- clients_public, like ClientQuietHoursCard does, because GATE A revoked base
-- clients SELECT from the authenticated role (agency reads route through this
-- security-definer view). The nudge columns are non-secret, so they belong in
-- the plain projection alongside cadence_quiet_hours. Body generated from the
-- live pg_get_viewdef so the full column list + tenant WHERE are reproduced
-- exactly, with only the 3 nudge columns appended.
...
CREATE OR REPLACE VIEW public.clients_public WITH (security_invoker=off) AS
 SELECT id,
    ...
    nudge_enabled,
    nudge_offsets_hours,
    nudge_recovery_window_hours
   FROM clients
  WHERE auth.role() = 'service_role'::text OR get_user_role(auth.uid()) = 'agency'::text AND (agency_id IN ( SELECT profiles.agency_id
           FROM profiles
          WHERE profiles.id = auth.uid())) OR get_user_role(auth.uid()) = 'client'::text AND id = get_user_client_id(auth.uid());
