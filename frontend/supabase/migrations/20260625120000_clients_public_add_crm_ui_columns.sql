-- 20260625120000_clients_public_add_crm_ui_columns.sql
-- types.ts drift fix (Session 2). Five UI-state columns were each added to clients
-- by an earlier repo migration, but never applied to the live DB (the project has
-- no migration runner — schema is applied via the Management API). So the frontend
-- reads/writes for them 400 against live, breaking five real features:
--   crm_page_size            (Contacts page-size persistence)        20260404152210
--   crm_column_widths        (Contacts/ErrorLogs column widths)      20260402212255
--   log_column_widths        (Logs column widths)                    20260407225001
--   sync_ghl_booking_enabled (GHL-booking sync toggle)               20260415121639
--   what_to_do_acknowledged  (onboarding "what to do" ack)           20251225151227
-- The browser reads these through public.clients_public (the B5/S1-1 security view),
-- which enumerates columns explicitly and therefore also omits them. All five are
-- non-secret, so appending them to clients_public preserves the secret-omission
-- boundary. This migration is idempotent: re-applying is a no-op.

BEGIN;

-- 1) Ensure the columns exist on the base table (exact types/defaults from the
--    original per-column migrations). IF NOT EXISTS keeps this safe to re-run.
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS crm_page_size integer NOT NULL DEFAULT 50;
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS crm_column_widths jsonb DEFAULT NULL;
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS log_column_widths jsonb DEFAULT NULL;
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS sync_ghl_booking_enabled boolean DEFAULT false;
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS what_to_do_acknowledged boolean DEFAULT false;

-- 2) Re-publish clients_public with the five non-secret columns appended. CREATE OR
--    REPLACE keeps the existing 115 columns in place (it only allows appending) and
--    preserves the security_invoker setting + existing grants. The leading column
--    list is reproduced verbatim from 20260624160000_clients_public_view.sql.
...
SELECT
  ...
  -- Non-secret UI-state columns (Session 2 drift fix), appended at the end so
  -- CREATE OR REPLACE only adds columns and never reorders/drops existing ones:
  crm_page_size,
  crm_column_widths,
  log_column_widths,
