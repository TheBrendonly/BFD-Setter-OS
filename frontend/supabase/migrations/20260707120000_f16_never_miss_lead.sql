-- F16 never-miss-a-lead pack — per-client opt-in flags + per-setter transfer target.
--
-- All default OFF (nothing auto-dials / texts back until the agency opts a client
-- in). Applied to LIVE prod via the Management API (this repo has no migration
-- runner; file is for record).
--
--   clients.speed_to_lead_enabled        F16(b): new lead -> AI voice call within
--                                        60s inside the legal window (SMS fallback
--                                        outside / on no-answer).
--   clients.missed_call_textback_enabled F16(c): an inbound call that abandons /
--                                        drops -> SMS back within 60s into the SMS
--                                        booking engine.
--
-- F16(d) live-transfer config stores nothing here: the transfer destination lives
-- inside the Retell agent's transfer_call tool (general_tools), edited in the UI
-- and flowed through retell-proxy untouched, so the Retell agent is the single
-- source of truth (no redundant DB column).
...
-- Recreate clients_public with the two new non-secret flags appended (full
-- definition required; includes the F17 recording_disclosure_enabled column).
