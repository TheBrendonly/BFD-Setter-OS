...
- Phase E1 (rebrand) ✅ DONE. Phase E2 (Lovable cleanup) ✅ DONE. E3 (Retell agents) ✅ DONE 2026-05-17 — full end-to-end voice booking PROVEN working: real GHL appointment `OfwxxJT9L5jO3IMGUTnn` created, `bookings` row `59780225-dfa5-49f7-95ca-c9f1ac486a1c` written via `bookings-webhook`. Required 6 stacked fixes (URL substitution + intake_lead_secret auto-mint + en-AU language + Stripe-optional gate + SyntaxError fix + phone-number agent_version repoint). E4 (setup-guide screenshots), E5 (pun-quiz rewrite) deferred.
- Cadence v2 draft at `engagement_workflows.c206da3e-...`, `is_active=false`; awaiting Brendan's eyeball + activation
- Latest handoff: `Operations/handoffs/2026-05-17-phase-e3-end-to-end-verified-handoff.md`
...
- **EE1. Voice AI Setter UI restructure: inbound/outbound mode toggle (architectural)** — Brendan's request. The current UI edits one "Voice-Setter-N" slot at a time but doesn't visually distinguish inbound vs outbound configuration intent. He wants: a main settings section at the top with an [Inbound] / [Outbound] toggle that controls which `clients.retell_*_agent_id` slot gets the push, AND a clear UI label so saving the inbound setter doesn't overwrite the outbound setter's config (and vice versa). Data model already supports 3 distinct slots (`retell_inbound_agent_id`, `retell_outbound_agent_id`, `retell_outbound_followup_agent_id`); just needs UI surfacing + retell-proxy `slotNumber` mapping verification. Scope: L (UI + retell-proxy interaction review).
- **EE2. retell-proxy auto-repoint phone-number agent_version after publish** — silent multi-tenant gap discovered tonight. `retell-proxy/sync-voice-setter` PATCHes the LLM + publishes the agent (creating v31, v32, v33…) but doesn't update `inbound_agent_version` / `outbound_agent_version` on the phone-number config. So every UI push silently fails to make tool changes live for actual phone calls — for every tenant. BFD's phone number had `inbound_agent_version: 30` pinned for hours after the multi-tenant tool URL fix shipped. Fix: in retell-proxy after `publish-agent`, query `clients.retell_phone_1/2/3`, PATCH each phone number's agent_version to the new published version (or set to null to always use latest). Scope: M.
- **EE3. 5 functions with legacy `deno.land/std@0.168.0/http/server.ts` imports** — Supabase's edge runtime now sets `--no-remote` which blocks legacy remote imports. Functions affected: `run-simulation`, `analyze-metric`, `analyze-setter-prompt`, `sync-external-credentials`, `stripe-portal`. All throw `worker boot error: failed to bootstrap runtime` and return 5xx. Fix: replace `import { serve } from "https://deno.land/std@0.168.0/http/server.ts"; serve(handler)` with `Deno.serve(handler)` (built-in, no import needed). Scope: S per function; deploy each via Management API.
- **EE4. External call_history sync failed permanently: 'public.call_history' not found in schema cache** — `retell-call-analysis-webhook` tries to mirror call_history rows to the external bfd-setter-live Supabase (`qildpilxjodxdifggmto`) but that project doesn't have a `call_history` table. Logged at every analyzed call. Cosmetic (call_history lands in bfd-platform fine) but noisy. Fix: either add a `call_history` table to bfd-setter-live with the matching schema, OR remove the external mirror call. Scope: S.
- **EE5. Voice paste-in still defaults to a different `custom_voice_xxx`** — the diagnostic `console.log` shipped in commit `8ba393e` (`retell-proxy/index.ts:144`) logs the raw `voiceSettings.voice_id` received from the UI on every push. Tonight's pushes should have captured this. Pull the function logs filtered by `[sync-voice-setter] voiceSettings.voice_id received:` to see whether the user's pasted value arrives correctly at retell-proxy — that pinpoints (a) UI state-staleness vs (b) sync-flow normalization. Scope: S (15 min diagnostic + 30 min fix).
