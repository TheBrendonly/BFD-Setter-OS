...
- HEAD: `9f8e2f5` on both Forgejo origin AND GitHub
...
- **EE3. ~~5 functions with legacy `deno.land/std@0.168.0/http/server.ts` imports~~ ✅ DONE 2026-05-17** in `phase-e3-followup-ee3-deno-serve-migration` (`ff08a8f`). Sweep covered ALL 30 affected functions (not just the 5 named — Brendan's call to do them all at once). Replaced `import { serve } from "..."` + `serve(handler)` with `Deno.serve(handler)`. Bonus: `process-lead-file` legacy base64 import migrated to `jsr:@std/encoding/base64`. All 30 deployed via Management API with `verify_jwt` preserved per-function via GET-then-PATCH. 30/30 ACTIVE.
