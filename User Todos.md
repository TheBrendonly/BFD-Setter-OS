...
- (a) Voice agent fetched slots in `America/New_York` despite `clients.timezone=Australia/Sydney`. Agent prompt or tool-arg default has ET baked in. **Confirmed re-occurring 2026-05-13** during the GHL form path test — pre_call_context.metadata.timezone = America/New_York.
...
- (e) **NEW 2026-05-13.** Twilio error extraction bug at `trigger/runEngagement.ts:171` — reads `error_code`/`error_message` from Twilio response but Twilio returns `code`/`message`. Today's failure showed as `"Twilio SMS failed: ? unknown"` instead of `"21610 Attempt to send to unsubscribed recipient"`. Trivial 15-min fix — change the field names, redeploy Trigger.dev.
