  ...
  run: async () => {
    ...
    for (const lead of (candidates ?? []) as Candidate[]) {
      ...
      if (tier >= nudgeCfg.offsetsHours.length) {
        ...
        // / cool-down stage instead of just dropping it. Resolve the lead's
        // workflow from its most recent execution (no pre-existing enrollment
        // row needed). Opt-in: no on_silent target = no-op. Non-fatal. The
        // tagged_silent flag above stops nudgeColdReply from re-touching the
        // lead; the long-tail cadence runs independently.
        try {
