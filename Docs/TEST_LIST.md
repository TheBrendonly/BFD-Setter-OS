...
- [ ] **F2c outbound config removed** — the voice setter editor no longer shows a "Phone Number" selector. A test outbound on BFD's cadence still dials (resolves from `retell_phone_1` fallback). Twilio import + phone-number management now lives on **API Credentials** (RetellPhoneNumbersTab, mounted under the Retell card) and on the Voice AI Rep Setup page.
