...
> **STILL OWED (next session, autonomous):** **API-DEPR-2(a)** (presets-on-agent after a clean voice-setter Save — headless Save button was elusive); **RUN 4** (F3 cadence pause/resume, F4 tz nudge-skip, FOLLOWUP-PROMPT-1 — Trigger flows); **RUN 7** (F1 fresh GHL contact deep-link); **F13 client-EYE view** (needs a client-role user → the Fable onboarding creates one). Test artifact left: synthetic lead `bfd-+61400000199` (harmless).
