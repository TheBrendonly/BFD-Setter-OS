...
## ⭐ Onboarding-fix pass 2026-07-06 — BUILT on `main`, live AFTER Brendan runs `git push github main` (Railway builds from GitHub; auto-mode blocked that push)
...
- [ ] **ONBOARD-1 live — a UI-created client is born with the SMS engine ON.** Create a throwaway sub-account via `/client/:id/create-client` (and once via the sidebar Add Sub-Account dialog), then SQL: `SELECT use_native_text_engine FROM clients WHERE id='<new>'` → `true` on both paths. Delete the throwaways after.
- [ ] **ONBOARD-2 live — create-setter guard + no orphan.** On a client WITHOUT external Supabase creds, click Create New Setter (either channel) → clear "External Supabase not configured … Credentials page first" toast, AND `SELECT count(*) FROM prompts WHERE client_id='<that client>'` unchanged (no orphan row). A text-prompt save on the same client shows the same clear message with no platform writes.
- [ ] **ACCESS-1 live — client login cannot reach the setter editors.** As a client-role login: `/prompts/voice` and `/prompts/text` redirect to the dashboard exactly like `/credentials`; the sidebar shows no Text Setter / Voice Setter items (check with a saved menu config AND the default menu).
- [ ] **GOLIVE-1 UI half — the Inbound Webhooks card explains readiness.** On a not-ready client the badge reads "Not go-live ready" and the new "Still missing: …" line lists the failing checks; the server halves (blank client false, dogfood client all-provisioning-true) were already verified live on webhook-manifest v3, 2026-07-06. NOTE: the dogfood client currently fails only `requiredWebhooksReceived` because `sync_ghl_executions` was created 2026-07-05 and has 0 rows yet; it flips true on the next GHL contact sync (honest signal, not a bug).
- [ ] **ONBOARD-3 live — password copy.** CreateClient + sidebar-dialog placeholders read "Min 12 characters"; a <12 password in the sidebar Add Sub-Account dialog is now REFUSED client-side (this used to reach admin createUser, which bypasses the GoTrue policy); Settings + ClientSettings password updates refuse <12 with matching copy.
