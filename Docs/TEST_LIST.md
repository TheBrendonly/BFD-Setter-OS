...
- [x] **F24 / v25 live VOICE booking — PASSED 2026-07-13** (call `call_9ad640407735916a081516f1ec2`, 108s). Main
  Outbound (agent_f45f4dd) booked cleanly end-to-end on v25 — real GHL appt + `bookings` row (source=voice_call),
  no ghost; test appt cancelled. F24 cadence-end not exercised (lead had no active cadence); code + unit-verified.
- [x] **PU-14 + PU-6 — APPLIED (Brendan) + VERIFIED on the same call.** PU-6: disclosure spoken in the opener.
  PU-14: agent booked via a real `book-appointments` call (no fabricated "booked"). → COMPLETED_LOG.
- [x] **SMS STOP/START — PASSED 2026-07-13 (autonomous).** A signed inbound STOP recorded an internal opt-out
  (`lead_optouts`, source=sms_stop); START cleared it; safety-net delete confirmed TEST_PHONE_A not left opted out.
- [ ] **SLOT-MAP-1 live UI refuse** (optional, belt-and-braces) — the guard is present in the deployed source
  (retell-proxy:754) + unit-tested; a live "Save & Push on the empty Setter-1 tile → refused" confirm is Brendan/UI.
