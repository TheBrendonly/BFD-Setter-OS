...
> **⭐⭐⭐ VOICE + BROWSER TEST SESSION — 2026-07-06 (hybrid: Claude drove browser+SMS via headless Playwright/harness, Brendan did the one answered voice call). ALL PASSED → `COMPLETED_LOG.md`** (full detail there + handoff `Operations/handoffs/2026-07-06-voice-browser-session.md`). RUN 0 green (git `380a889`; vbt v23 / retell-proxy v49 / webhook-manifest v3 / ach v19 / av2p v19 / ca v16 ACTIVE; test:node 127/127, edge 217/217).
> **Browser (agency session):** Onboarding-fix rows **ONBOARD-1** (both create paths → `use_native_text_engine=true`), **ONBOARD-2** (guard toast + no orphan), **ONBOARD-3** (Min-12 copy + <12 refused), **GOLIVE-1 UI** ("Still missing" line), **ACCESS-1** (client-role: /prompts/voice + /prompts/text + /credentials all redirect to dashboard, sidebar trimmed) — all PASS, throwaways cleaned up. **SWEEP-1a/b/c** (no clients_public 400, no logs uuid-400, no chat_starred/dismissed_error_alerts 404; star+dismiss persistence + client-role RLS own-row writes proven via PostgREST). **F9-1** (locked tile rename refused + doc-page unreachable, no setter_display_names write). **PHONE-CLEAR-1** (add/edit/clear via Contacts dialog + ContactDetail + Chats panel → normalized_phone follows/nulls/sets). **G3-7** (vite-8 nav of 6 routes, zero module errors).
...
> (webhook-manifest v3) is ALREADY LIVE and server-verified; the rows below are the frontend/UI halves.
> The agency storageState in `scripts/test-harness/` drives most of these headlessly; ACCESS-1 needs a
> client-role login (create a throwaway client user, delete after, per the 2026-07-06 gate-run pattern).

- [x] **ONBOARD-1 live — PASS 2026-07-06 (both create paths → `use_native_text_engine=true`). — a UI-created client is born with the SMS engine ON.** Create a throwaway sub-account via `/client/:id/create-client` (and once via the sidebar Add Sub-Account dialog), then SQL: `SELECT use_native_text_engine FROM clients WHERE id='<new>'` → `true` on both paths. Delete the throwaways after.
- [x] **ONBOARD-2 live — PASS 2026-07-06 (clear guard toast + zero orphan prompts rows). — create-setter guard + no orphan.** On a client WITHOUT external Supabase creds, click Create New Setter (either channel) → clear "External Supabase not configured … Credentials page first" toast, AND `SELECT count(*) FROM prompts WHERE client_id='<that client>'` unchanged (no orphan row). A text-prompt save on the same client shows the same clear message with no platform writes.
- [x] **ACCESS-1 live — PASS 2026-07-06 (client-role: /prompts/voice + /prompts/text + /credentials redirect to dashboard; no Text/Voice Setter sidebar items). — client login cannot reach the setter editors.** As a client-role login: `/prompts/voice` and `/prompts/text` redirect to the dashboard exactly like `/credentials`; the sidebar shows no Text Setter / Voice Setter items (check with a saved menu config AND the default menu).
- [x] **GOLIVE-1 UI half — PASS 2026-07-06 ("Not go-live ready" badge + "Still missing" line lists failing checks). — the Inbound Webhooks card explains readiness.** On a not-ready client the badge reads "Not go-live ready" and the new "Still missing: …" line lists the failing checks; the server halves (blank client false, dogfood client all-provisioning-true) were already verified live on webhook-manifest v3, 2026-07-06. NOTE: the dogfood client currently fails only `requiredWebhooksReceived` because `sync_ghl_executions` was created 2026-07-05 and has 0 rows yet; it flips true on the next GHL contact sync (honest signal, not a bug).
- [x] **ONBOARD-3 live — PASS 2026-07-06 (Min-12 placeholders on both create paths; <12 refused client-side in the sidebar dialog). — password copy.** CreateClient + sidebar-dialog placeholders read "Min 12 characters"; a <12 password in the sidebar Add Sub-Account dialog is now REFUSED client-side (this used to reach admin createUser, which bypasses the GoTrue policy); Settings + ClientSettings password updates refuse <12 with matching copy.

- [x] **VOICE GATE — PASS 2026-07-06 (voice + browser session): answered outbound booking on v49+v23, `call_c347226e…`, booked GHL `yw7NyOE0…` (real slots 12:00/12:30 Mon Jul-13, no fabrication), `bookings.source='voice_call'`, clean hangup; reschedule+cancel bound the real id (CANCEL-1 voice). No regression vs the 2026-07-03 baseline. → COMPLETED_LOG. — retell-proxy v49 answered-call booking regression (retell-proxy is the FROZEN Voice baseline; v49 SUPERSEDES v48 — API-DEPR-2 landed 2026-07-04).** Read-only smoke already PASSED on both v48 (POST `/v2/list-agents`→24 agents + get-agent hydration) and v49 (list-agents 200 + canonical agents byte-for-byte unchanged), 0 agents mutated. Owed: place ONE answered outbound booking call (Main Outbound / Voice-Setter-master), run the booking to completion → confirm the call connects + books exactly as on v47 (compare to the 2026-07-03 gate `call_d5625539` / booking `4f7c76a0`). If it regresses, roll retell-proxy back to v47 (or v48) via `deploy_single_fn.mjs`. Pairs with the VM-1 voicemail-lands check below (do both on the same call session where practical).
- [x] **API-DEPR-2 — analysis fields migrated to system-presets (retell-proxy v49). BOTH PARTS PASS.** **(a) PASSED 2026-07-06 (Fable onboarding run):** pushed `sync-voice-setter` with a real AGENCY user JWT on a fresh THROWAWAY client → created agent `agent_c09e76046be7e61b57c030104d`; `get-agent` showed `post_call_analysis_data` = 3 `type:"system-presets"` (`call_summary`/`call_successful`/`user_sentiment`) + 6 custom fields, NO dupes, the 3 deprecated `analysis_*_prompt` fields ABSENT, born-bookable (5/5 booking tools, per-tenant voice-booking-tools URL). Throwaway agent + client deleted after. → COMPLETED_LOG. **(b) PASSED live 2026-07-05** (top-level analysis populated on the answered call). _Original carry note:_ `retell-proxy` requires a real user JWT (no service-key path) so it cannot be replayed server-side; Fable did a voice-setter push on a fresh THROWAWAY agent and asserted the presets there. Two live checks (both zero-risk to the conversation; post-call REPORTING only): (a) after Brendan re-Saves any setter through the BFD setter UI, `get-agent` on that agent shows the 3 deprecated `analysis_*_prompt` fields still absent and `post_call_analysis_data` now carrying 3 `type:"system-presets"` entries (`call_summary`/`call_successful`/`user_sentiment`) + the existing custom fields, no dupes; (b) on the VOICE-GATE answered call above, confirm `call_summary` / `user_sentiment` / `call_successful` still populate TOP-LEVEL in `call_history` after the call (proves the migration kept top-level output, so the analysis webhooks are unaffected). Also: over the next sweep, confirm Retell's legacy-list + analysis-prompt deprecation notices stop firing.
...
> **STILL OWED — UPDATED 2026-07-06 (Fable onboarding run, autonomous):** **RUN 4** (F3 / F4 / FOLLOWUP-PROMPT-1) and **RUN 7** (F1) DONE + PASS. **API-DEPR-2(a)** and the **F13 client-EYE view** are now BOTH DONE + PASS on the Fable onboarding run → `COMPLETED_LOG.md`. F13 client-eye: created a real client-role login on a throwaway client; `get-client-usage` as the CLIENT jwt = server-enforced whitelist (ALL OFF → `{show:false}`; each toggle exposes only its own figure; ALL ON → all four; AGENCY jwt still gets the full margin payload). UI as the client on `/account-settings`: ALL ON → "Usage & Billing" card with rate `/min` + Month total; ALL OFF → card ABSENT. Throwaway client + user + agent deleted after. Nothing now blocked on a throwaway agent/client.
...
- [x] **CANCEL-1: SMS + voice cancel/reschedule bind a real eventId. — PASS 2026-07-06 → COMPLETED_LOG (SMS cancel bound real id + all fabricated ids refused; voice reschedule+cancel bound real id, no 404, time moved + status flipped; protected appt untouched). NEW model/prompt finding RESCHED-SMS-1 tracked separately (BUG_LIST).** _SMS BOOK half exercised 2026-07-06 (book cycle clean, no 404); the CANCEL/RESCHEDULE half was NOT run autonomously — TEST_PHONE_A holds a live confirmed Jul-8 appt, so an unattended misbind could cancel a real one. Cancel/reschedule (SMS + voice) → the supervised voice session (Prompt 2)._ After deploying voice-booking-tools
...
- [x] **SMS-METER-1: mid-call text meters. — PASS 2026-07-06 → COMPLETED_LOG (voice in-call send-sms → message_queue sms_outbound row, sid SM82b1cf…, BFD location, processed; SMS direct-tool half passed part 1).** _Direct-tool half PASSED 2026-07-06: a `send-sms` tool POST (intake bearer) to TEST_PHONE_A stamped a new `message_queue` `channel='sms_outbound'` row (ghl_account_id=BFD location, twilio_message_sid=SM449634…, processed). The in-call VOICE path (agent sends mid-call SMS) owed → Prompt 2._ After deploy, have a voice agent send a mid-call SMS → a
