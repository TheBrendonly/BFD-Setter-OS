...
- [x] **Overnight Text-Setter repair — BUILT + STAGED 2026-07-01** on branch `worktree-overnight+text-setter-repair-allbugs` (local-only, not deployed). All open bugs coded/staged; tests green; adversarial council DONE-CONFIRMED.
- [ ] **DAYLIGHT — deploy the two staged branches (Session 7-finish).** **DEPLOY NOTHING ran overnight; two branches await you, present + alert.** (1) `worktree-overnight+text-setter-repair-allbugs` FIRST — use its deploy checklist in handoff `2026-07-01-overnight-text-setter-repair-allbugs.md`: deploy the non-Voice targets (Trigger redeploy + `tool_invocations` migration + frontend + `execute-lead-webhook`), then **VM-1 `retell-proxy v46→v47` behind the Voice-regression gate** (place a real outbound call → booking works → B-3/B-5 survive). (2) The F8 branch, once you've run its build prompt (`2026-06-30-f8-build-kickoff-prompt.md`) — F8 is NOT Voice-gated. Then run the live acceptance + still-owed Session-7-finish tests (Claude verifies read-only; you place the one call + the test SMS). Apply the BOOK-1 prompt tweak below **before** the 3.12 re-test. `[B]`
