...
- [ ] **Provision the F1 "BFD Conversation Link" GHL custom field (activates F1)** — F1 shipped Session 4 (`sync-ghl-contact` v24 writes the lead's BFD conversation link onto the GHL contact on create), but it's **dormant** until the target field exists. In GHL → the location's Custom Fields, create a **text/URL** field named "BFD Conversation Link"; copy its field id; then store it: `UPDATE clients SET ghl_conversation_link_field_id='<id>' WHERE id='<client>'` (Mgmt API / DB). Until then every new lead's `sync-convo-link` step shows "skipped" (harmless). Then run the F1 test in TEST_LIST. `[B]`
