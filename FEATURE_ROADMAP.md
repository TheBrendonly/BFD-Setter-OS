...
- [~] **2.2 Engagement editor email-channel picker**: email channels work in the JSON / runtime but the canvas channel picker does not render an "email" option yet. UI-only. Source: `User Todos.md` CV2-5.
...
- [~] **2.5 Credential "Verify" button** per provider group (Retell list-agents, GHL location GET, Twilio account fetch, OpenRouter models): cheap read, pass/fail inline, prevents silent dead-on-arrival clients. High demo value. Source: `Docs/AUDIT_2026-06-10_full-system-audit.md`.
...
- [ ] **3.4 Pause / resume on a running cadence**: add a `paused` state to `engagement_executions` plus a UI button and state-machine logic in `runEngagement.ts`, so an owner can pause a campaign for a holiday week without losing the lead's place. Source: `Docs/CAMPAIGN_PLAYBOOK.md` Gap E5.
...
- [ ] **3.8 Brand voice prompt overrides**: `clients.brand_voice` column + per-workflow override; `aiGenerateEngagementCopy` already accepts `brandVoice`, just wire it from the DB. Effort: S. Source: `User Todos.md` CV2-7.
- [ ] **3.9 Cost ceiling per-week / per-month aggregates**: today's guard is per-lead (>500c to error_logs). Add a per-tenant rolling-window aggregate so a runaway tenant is flagged before any single lead is. Effort: S. Source: `User Todos.md` CV2-8.
